...
사정이 있을 경우 미리 slack으로 알려주시고, 병결하신 경우엔 다음날에라도 꼭 알려주세요!<br>
의견을 제시할 때는 근거와 함께 최대한 논리적으로 설명해주세요.<br>
피드백은 상호 존중하며 쿠션어를 사용해주세요.<br>
전체 회의는 특별한 일이 없으면 매일(평일만) 총 2번 진행합니다.<br>

### 오전 회의 : 간단하게 10분 내외로 진행합니다.<br>
시간 : 10시 (상황에 따라 변경될 수 있음)<br>
...
## branch 전략
`main` 브랜치 : `dev` 브랜치에서 배포할 수 있을 정도로 구현된 것을 Merge 후 배포하는 브랜치<br>
`dev` 브랜치 : 기능 개발과 버그 수정 브랜치와 Merge가 자주 일어나는 브랜치이다. 개발이 가장 활발하게 일어나는 브랜치<br>
`feat/기능이름` or `fix/main-page` : 이슈나 기능에 대해 개발하는 브랜치, 구현이 끝나면 `dev` 브랜치에 Merge<br>
** 브랜치 이름을 만들 때 가능하면 대문자 쓰지 말기!!**
